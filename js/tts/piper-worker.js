...
self.onmessage = async (e) => {
    const { type, text, voiceId, index, sessionId, workerId: id } = e.data;
    ...
    if (type === 'SYNTHESIZE') {
        try {
            console.log(`[Worker-${workerId} synth start]`, text);
            ...
            console.log(`[Worker-${workerId} synth predict] ${useTime}ms (${useTime / input.length}ms/char)`, input);

            const arrayBuf = await wavBlob.arrayBuffer();
            self.postMessage({ type: 'DONE', index, sessionId, buffer: arrayBuf, text }, [arrayBuf]);
        } catch (err) {
            console.error(`[Worker-${workerId}] SYNTHESIZE error:`, err);
